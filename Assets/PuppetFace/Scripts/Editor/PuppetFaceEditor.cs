	...
	public class PuppetFaceEditor : EditorWindow
	{
		...
		public static void ShowWindow()
		{
			...
			if (blendShapeTextures.Count == 0)
			{
				for (int i = 0; i < 10; i++)
				{
					string faceShape = ("Assets/PuppetFace/Textures/GUI/img/lisa" + i + ".png");
		...
		private void OnEnable()
		{
			...
			string phonemeMarkerPath = "Assets/PuppetFace/Textures/GUI/phonemeMarker.png";
		...
		void OnGUI()
		{
			...
			string BGPath = "Assets/PuppetFace/Textures/GUI/BG.png";
			string BGDarkPath = "Assets/PuppetFace/Textures/GUI/BGDark.png";
			...
			if (GUILayout.Button("Convert Audio Files"))
			{
				ConvertAudioToLipSync();
			}
			...
			if (LipSyncLoaded != null)
			{
				...
				if (AudioSourceLoaded != null && AudioClipLoaded != null && AudioClipNames != null && AudioClipNames.Length > 0)
				{
					...
					Texture PlayButtonAsset = AssetDatabase.LoadAssetAtPath("Assets/PuppetFace/Textures/GUI/PlayButton.png", typeof(Texture)) as Texture;
					Texture PlayButtonOnAsset = AssetDatabase.LoadAssetAtPath("Assets/PuppetFace/Textures/GUI/PlayButtonOn.png", typeof(Texture)) as Texture;
					Texture StopButtonAsset = AssetDatabase.LoadAssetAtPath("Assets/PuppetFace/Textures/GUI/StopButton.png", typeof(Texture)) as Texture;
					Texture StopButtonOnAsset = AssetDatabase.LoadAssetAtPath("Assets/PuppetFace/Textures/GUI/StopButtonOn.png", typeof(Texture)) as Texture;

					GUIStyle buttonStyle = new GUIStyle();
					buttonStyle.stretchWidth = true;
					buttonStyle.stretchHeight = true;

					if (GUI.Button(new Rect(widthPad + 30, heightPad + 150, 32, 32), Playing ? PlayButtonOnAsset : PlayButtonAsset, buttonStyle))
					{
					...
					if (GUI.Button(new Rect(widthPad + 62, heightPad + 150, 32, 32), Playing ? StopButtonAsset : StopButtonOnAsset, buttonStyle))
					{
						if (!Application.isPlaying)
						{
					...
					TimeVal = float.Parse(GUI.TextField(new Rect(widthPad + 94, heightPad + 150, 100, 32), TimeVal.ToString(), textFieldStyle));
				...
				string LogoPath = "Assets/PuppetFace/Textures/GUI/PuppetFaceLogo.png";
		...
		private void ConvertAudioToLipSync()
		{
			var extensions = new[] {
				...
				foreach (string path in files)
				{
					...
					if (Selection.activeGameObject != null)
					{
						...
						if (lipSync == null)
						{
							...
							lipSync = Selection.activeGameObject.AddComponent<LipSync>();
							lipSync.Skin = smr;
						}

						List<AudioClip> audioClips = new List<AudioClip>();
						if (lipSync.AudioClips != null && lipSync.AudioClips.Length > 0)
							audioClips = lipSync.AudioClips.ToList();
						audioClips.Add(audioCip);
						lipSync.AudioClips = audioClips.ToArray();

						AssetDatabase.Refresh();

						List<TextAsset> lipSyncFiles = new List<TextAsset>();
						if (lipSync.LipSyncFiles != null && lipSync.LipSyncFiles.Length > 0)
							lipSyncFiles = lipSync.LipSyncFiles.ToList();
						...
						lipSync.LipSyncFiles = lipSyncFiles.ToArray();
						AudioSourceLoaded = Selection.activeGameObject.GetComponent<AudioSource>();
						LipSyncLoaded = lipSync;
						LipSyncLoaded.LipSyncIndex = LipSyncLoaded.LipSyncFiles.Length - 1;
						LipSyncLoaded.Initialized = false;
						LipSyncLoaded.InitializeFromData(MakeFaceShapeData(_phonemeMarkers), LipSyncLoaded.LipSyncIndex);
			...
			if (LipSyncLoaded != null)
			{
				...
				if (LipSyncLoaded.FaceShapes != null && LipSyncLoaded.FaceShapes.Count > LipSyncLoaded.LipSyncIndex)
				{
					...
					for (int i = 0; i < _phonemeMarkers.Count; i++)
					{
						...
						newFaceShape.Add(new FaceShape(_phonemeMarkers[i].Start, _phonemeMarkers[i].End, _phonemeMarkers[i].Label, cue.strength));
					}
		...
		private static void CreateLipSyncBlendShapeButton(int id, int widthPad, int heightPad, string[] BlendShapeNames)
		{
			...
			string faceShape = ("Assets/PuppetFace/Textures/GUI/img/lisa" + id + ".png");
		...
		private void OnClickAddNode(Vector2 mousePosition, string shapeName)
		{
			...
			_phonemeMarkers.Add(new PhonemeMarker(new Vector2(markerPos, WaveFormPosY + 100), 70, 70, _phonemeMarkerStyle, shapeName, markerPos - 0.05f, markerPos + 0.05f, 1f, _phonemeMarkers.Count));
		...
		private void ReloadNodes(Vector2 mousePosition)
		{
			...
			if (AudioSourceLoaded && LipSyncLoaded && AudioClipLoaded && LipSyncLoaded.LipSyncFiles.Length > 0)
			{
				...
				for (int i = 0; i < _phonemeArray.Length; i++)
				{
					...
					_phonemeMarkers.Add(new PhonemeMarker(new Vector2(WaveFormPosX - 35 + (timeVal / AudioClipLoaded.length) * WaveFormWidth, WaveFormPosY + 100), 70, 70, _phonemeMarkerStyle, vals[2], float.Parse(vals[0]), float.Parse(vals[1]), float.Parse(vals[3]), i));
			...
			if (AudioSourceLoaded && LipSyncLoaded && AudioClipLoaded)
			{
				...
				else
				{
					...
					for (int i = _phonemeMarkers.Count - 1; i >= 0; i--)
					{
						...
						_phonemeMarkers[i] = (new PhonemeMarker(new Vector2(WaveFormPosX - 35 + (timeVal / AudioClipLoaded.length) * WaveFormWidth, WaveFormPosY + 100), 70, 70, _phonemeMarkerStyle, _phonemeMarkers[i].Label, _phonemeMarkers[i].Start, _phonemeMarkers[i].End, _phonemeMarkers[i].Strength, i));
		...
		public ListFaceShapes MakeFaceShapeData(List<PhonemeMarker> pms)
		{
			...
			{
				FaceShapeData.Add(new FaceShape(pms[t].Start, pms[t].End, pms[t].Label, pms[t].Strength));
