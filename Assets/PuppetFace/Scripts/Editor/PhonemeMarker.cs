...
namespace PuppetFace
{
	public class PhonemeMarker
	{
		...
		public PhonemeMarker(Vector2 position, float width, float height, GUIStyle nodeStyle, string label, float start, float end, float strength, int index)
		{
		...
		public void Draw()
		{
			...
			{
				float shapeDuration = (End - Start);
				float end = (End - Start) * (1.35f - 0.35f * Mathf.Clamp01(shapeDuration)) + Start;
				float start = (Start - End) * (1.35f - 0.35f * Mathf.Clamp01(shapeDuration)) + End;
			...
			switch (e.type)
			{
				case EventType.MouseDown:
					if (e.button == 0)
					{
						...
						if (reducedRect.Contains(e.mousePosition))
						{
							isDragged = true;
							GUI.changed = true;
